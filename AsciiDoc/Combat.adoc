...
Combats are organized in [lexicWord]#rounds#.
A [lexicWord]#round# is about 6 seconds (there are 10 rounds in a minute).
This is more of an average than a fix measure.

Every [lexicWord]#round#, the [lexicWord]#GM# should called [lexicWord]#initiative number# from the highest [lexicWord]#initiative# counting down until every [lexicWord]#character# had their turn.

Each [lexicWord]#character# can perform one or two [lexicWord]#actions# and one [lexicWord]#movement# on their [lexicWord]#initiative number#.

[lexicWord]#Characters# with an [lexicWord]#initiative# greater than 0 can decide to delay their turn to the negative of their [lexicWord]#initiative number#.
...
If some [lexicWord]#characters# are surprised at the start of a combat, they roll [lexicWord]#initiative# normally but can only act on the first [lexicWord]#round# at the negative of their roll, if this one is greater than 0.
If they roll 0 or less, they can only act from the second [lexicWord]#round#.

The number of dice used to roll [lexicWord]#initiative# are also deduced from any other [lexicWord]#dice-pool# the surprised [lexicWord]#characters# are going to use during the first [lexicWord]#round#.

'''

== Actions

Every [lexicWord]#round#, each [lexicWord]#character# can move up to their [lexicWord]#movement speed# and do one or two of the following actions:
...
[lexicWord]#Character# with an [lexicWord]#initiative# greater than 0 can perform two actions in a [lexicWord]#round#.
The first action must be performed at their normal [lexicWord]#initiative number# while the second action will be performed at the negative of their [lexicWord]#initiative#.

Some situations can also allow a [lexicWord]#character# to perform a second action, regardless of their [lexicWord]#initiative#, as a [lexicWord]#reaction#.
[lexicWord]#Reactions# are described further in this chapter.

If a [lexicWord]#character# decide to perform two actions instead of one, they must split their [lexicWord]#dice-pool# between the two actions.

'''

== Dice Pool Splitting

If a [lexicWord]#player# wants their [lexicWord]#character# to perform two actions in a [lexicWord]#round#, they must split their [lexicWord]#dice-pool# between the two actions.

The total number of dice used for both actions cannot be greater than the number of dice granted by the actions allowing the actions giving the biggest [lexicWord]#dice-pool#.
Only the number of dice and not the type of dice is of concern in [lexicWord]#dice-pool# splitting.

.Example
[caption = ""]
====
A [lexicWord]#player# wants to [lexicWord]#intimidate# a target before attacking it.
The [lexicWord]#character’s# [lexicWord]#insult & intimidation skill# grant them a [lexicWord]#dice-pool# of 3d6 while their [lexicWord]#melee attack skill# allow them a [lexicWord]#dice-pool# of 5d8.
The [lexicWord]#player# decide to use 2d6 for the [lexicWord]#intimidation# roll, which they will do at their [lexicWord]#initiative number#.
That would leave them 3d8 for their attack, which will be done at the negative of their [lexicWord]#initiative number#.
====
...
[lexicWord]#Free actions# are actions which do not count as part of the two actions allowed in a [lexicWord]#round#.
They are either very swift actions or actions which can be performed simultaneously to other actions or [lexicWord]#movement#.

Talking, drawing a weapon (but not sheathing) or dropping a held object are example of [lexicWord]#free actions#.

[lexicWord]#Free actions# do not require a test; If a task require a test, it is not a [lexicWord]#free action#.

'''

== Reactions

[lexicWord]#Reactions# are actions which can be performed out of the [lexicWord]#initiative# order, when some conditions are met.

[lexicWord]#Reactions# do not count as part of the two allowed actions but any dice used in a [lexicWord]#reaction# must be subtracted from the [lexicWord]#dice-pools# of any other action performed as per the [lexicWord]#dice-pool# splitting rule.

[lexicWord]#Reactions# are very common in a [lexicWord]#round#.
The most common type of [lexicWord]#reactions# are:

* Defending against a melee attacked

* Avoiding a range attack

* Resisting [lexicWord]#intimidation# or a [lexicWord]#spell#

* [lexicWord]#Attack of opportunity#

More details further in this chapter.
...
Every weapon has at least one [lexicWord]#range of attack# but some are effective at a variety of [lexicWord]#range of attack#.
...
If a weapon is noted with a [lexicWord]#range of attack# followed by a negative number, the weapon can still be used at this range (and cause damage) but [lexicWord]#MoSes# are subtracted from any attack roll made at this range.
...
Every weapon effective at far range will be noted with a number next to the far range mention.
This number represents the range increment in metres.
